  ...
  <div>
    ...
    <q-card class="q-mx-auto my-card bg-primary text-white">
      <q-card-section>
        ...
        <q-card-section>
          <div class="text-h6  text-black">Add new sub-category to {{item.name}}</div>
        </q-card-section>
    ...
    >
      ...
      <q-item v-for="n in items" :key="n.id">
        <q-item-section @click="$router.push('/adminemy12/list/'+n.name)">
          {{n.name}}
        </q-item-section>
        <q-item-section side>
    ...
    run () {
      ...
      http({
        ...
        .then((response) => {
          ...
          this.items = response.data.data[0].subcategory
          this.visible = false
        }).catch(function (error) {
          alert(error)
          this.visible = false
