<template>
  <div>
      ...
      >
        <q-tab v-for="n in subcategory" class="text-capitalize text-weight-bolder" :key="n.id" @click="getCat(n.name)" :name="n.name" :label="n.name" />
         </q-tabs>
...
>
           ...
           v-if="m.default === '1'"
              ...
              <p v-if="n.discount" style="color: #fff;
                ...
                padding: 5px;
                border-radius: 10px;
                background-color: #FF9800;">-{{n.discount}}%</p>
              </q-img>
              </div>
...
export default {
  ...
  methods: {
    ...
    getSubCat () {
      ...
      http({
        ...
        .then((response) => {
          this.subcategory = response.data.data[0].subcategory
          this.tab = this.subcategory[0].name
          this.getCat(this.tab)
        })
    }
  },
  ...
  mounted () {
    if (this.$route.params.subcategory && this.subcategory.length) {
      this.getCat(this.$route.params.subcategory)
    } else {
      this.getSubCat()
    }
  }
